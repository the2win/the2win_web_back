...
let nonceCounter = 1;
function generateOutcome(serverSeed, nonce) {
...
export class BoxesService {
    ...
    async play(userId, amount, chosenIndex) {
        if (amount <= 0)
        ...
        try {
            ...
            balanceAfter = Number(betTx.balance);
        }
        catch (e) {
        ...
        let multiplierAwarded;
        if (chosenIndex === outcome.winIndex2x)
            multiplierAwarded = 2;
        else if (chosenIndex === outcome.winIndex3x)
            multiplierAwarded = 3;
        else if (chosenIndex === outcome.winIndex5x)
            multiplierAwarded = 5;
        if (multiplierAwarded) {
            ...
            try {
                ...
                balanceAfter = Number(winTx.balance);
            }
            catch {
