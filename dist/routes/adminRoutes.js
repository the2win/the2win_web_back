...
router.post('/overrides/boxes', async (req, res, next) => {
    try {
        ...
        if ('winners' in req.body) {
            ...
            if (idxSet.size !== winners.length)
                return res.status(400).json({ message: 'Duplicate winner indexes not allowed' });
            payload = { winners };
        }
        else {
...
router.post('/withdraw-requests/:id/reject', async (req, res, next) => {
    try {
        ...
        try {
            await (await import('../services/walletService.js')).addTransaction(rec.userId, 'DEPOSIT', Number(rec.amount), { method: rec.method, withdrawRequestId: rec.id, refund: true });
        }
        catch (refundErr) {
