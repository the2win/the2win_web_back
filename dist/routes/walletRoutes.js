...
router.post('/bank-accounts', requireAuth, async (req, res, next) => {
    try {
        ...
        const acct = await BankAccount.create({ userId: String(req.user.id), bankName, accountNumber, accountHolder, isDefault: makeDefault ? 1 : 0 });
        if (makeDefault) {
            await BankAccount.update({ isDefault: 0 }, { where: { userId: String(req.user.id), id: { [Op.ne]: acct.id } } });
        }
...
router.post('/bank-accounts/:id/default', requireAuth, async (req, res, next) => {
    try {
        ...
        await BankAccount.update({ isDefault: 0 }, { where: { userId } });
        acct.isDefault = 1;
        await acct.save();
        res.json({ ok: true });
    }
...
router.get('/deposit-requests', requireAuth, async (req, res, next) => {
    try {
...
router.get('/withdraw-requests', requireAuth, async (req, res, next) => {
    try {
...
router.post('/deposit', requireAuth, async (req, res, next) => {
    try {
        ...
        if (amount < 1000 || amount > 500000)
            return res.status(400).json({ message: 'Amount must be between Rs. 1000 and Rs. 500000' });
        ...
        const rec = await DepositRequest.create({ id: nanoid(), userId: String(req.user.id), amount: amount, method: method, receiptPath });
...
router.post('/withdraw', requireAuth, async (req, res, next) => {
    try {
        ...
        if (amount < 1000 || amount > 200000)
            return res.status(400).json({ message: 'Withdraw amount must be between Rs. 1000 and Rs. 200000' });
        ...
        const todays = await WithdrawRequest.findAll({ where: { userId: String(req.user.id), createdAtMs: { [Op.gte]: since } } });
        const todaysTotal = todays.reduce((sum, r) => sum + Number(r.amount || 0), 0);
        ...
        if (method === 'bank_in') {
            if (!bankAccountId)
                return res.status(400).json({ message: 'bankAccountId required' });
        ...
        let finalDest = null;
        if (method === 'bank_in') {
            finalDest = req.body.dest; // constructed above
        }
        ...
        try {
            ...
            const rec = await WithdrawRequest.create({ id: requestId, userId: String(req.user.id), amount: amount, method: method, dest: finalDest });
...
router.get('/summary', requireAuth, async (req, res, next) => {
    try {
        ...
        try {
            const [rows] = await (await import('../config/db.js')).pool.query(`SELECT UPPER(type) AS type, SUM(amount) AS total
         FROM transactions
         WHERE user_id = ?
        ...
        const out = { DEPOSIT: 0, WITHDRAW: 0, BET: 0, WIN: 0 };
        for (const t of txs)
            out[String((t.type || '').toUpperCase())] = (out[String((t.type || '').toUpperCase())] || 0) + Number(t.amount || 0);
        res.json(out);
    }
    catch (e) {
        next(e);
