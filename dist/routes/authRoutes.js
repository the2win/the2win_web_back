    ...
    try {
        ...
        if (!user) {
            try {
                ...
                if (Array.isArray(rows) && rows.length)
                    user = { id: String(rows[0].id), email: rows[0].email };
            }
            catch { }
        }
        if (!user)
...
router.post('/verify-otp', async (req, res, next) => {
    try {
        ...
        let user = undefined;
        try {
            const [rows] = await pool.query(`SELECT id,email,otp_code as otpCode, otp_expires_at as otpExpires FROM users WHERE email=? LIMIT 1`, [email]);
            if (Array.isArray(rows) && rows.length)
                user = { id: String(rows[0].id), email: rows[0].email, otp: { code: rows[0].otpCode, expiresAt: Number(rows[0].otpExpires) } };
        }
        catch { }
        if (!user)
            user = db.users.find(u => u.email === email);
