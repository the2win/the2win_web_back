...
router.get('/history', async (_req, res) => {
    ...
    if (mem.length)
        return res.json({ history: mem });
    try {
        const [rows] = await pool.query(`SELECT round_id as roundId, crash_point as crashPoint, crashed_at as crashedAt FROM crash_rounds WHERE crashed_at IS NOT NULL ORDER BY round_id DESC LIMIT 25`);
        return res.json({ history: rows.map(r => ({ roundId: Number(r.roundId), crashPoint: r.crashPoint !== null ? Number(r.crashPoint) : null, crashedAt: r.crashedAt })) });
    }
    catch {
...
router.get('/stream', (req, res) => {
    ...
    send();
    req.on('close', () => {
        crashGameService.removeListener('update', listener);
    });
});
router.post('/bet', requireAuth, async (req, res) => {
    ...
    const result = await crashGameService.placeBet(userId, parsed.data.amount, parsed.data.slot || 'A');
    if (!result.ok) {
...
router.post('/cashout', requireAuth, async (req, res) => {
    ...
    const parsed = schema.safeParse(req.body);
    const userId = req.user.id;
    const result = await crashGameService.cashOut(userId, parsed.success ? parsed.data.slot : undefined);
    if (!result.ok)
        return res.status(400).json({ message: result.message || 'Cashout failed' });
