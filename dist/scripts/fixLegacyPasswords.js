...
export async function runFixLegacyPasswords() {
    ...
    const [colCheck] = await pool.query(`SELECT COUNT(*) AS hasLegacy FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'password'`);
    const hasLegacy = Array.isArray(colCheck) && colCheck.length && Number(colCheck[0].hasLegacy) > 0;
    if (!hasLegacy) {
    ...
    const [rows] = await pool.query(`SELECT id, email, password AS legacyPass, password_hash AS passwordHash
     FROM users 
    ...
    const [hashRows] = await pool.query(`SELECT id, password_hash AS passwordHash FROM users WHERE password_hash IS NOT NULL AND password_hash != ''`);
