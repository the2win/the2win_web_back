...
export async function ensureAdminAccount(opts: AdminBootstrapOptions) {
  ...
  try {
    ...
    const [rowsByEmail] = await pool.query<any[]>(`SELECT id, email, role, password_hash AS passwordHash, balance FROM users WHERE email=? LIMIT 1`, [email]);
    let existing = Array.isArray(rowsByEmail) && rowsByEmail.length ? rowsByEmail[0] : undefined;
    if (!existing) {
      const [rowsAnyAdmin] = await pool.query<any[]>(`SELECT id, email, role, password_hash AS passwordHash, balance FROM users WHERE role='admin' LIMIT 1`);
      if (Array.isArray(rowsAnyAdmin) && rowsAnyAdmin.length) existing = rowsAnyAdmin[0];
    }
    ...
    console.warn('[adminBootstrap] Lookup failed, will attempt insert anyway:', (e as any)?.message || e);
  }
  ...
  try {
    await pool.query(
      'INSERT INTO users (email, password_hash, role, balance, created_at, updated_at) VALUES (?, ?, "admin", 0, NOW(), NOW())',
      [email, password_hash]
    );
    ...
    if (msg.includes("doesn't have a default value") && msg.includes('id')) {
      ...
      const nextId = Array.isArray(maxRows) && maxRows.length ? (maxRows as any)[0].nextId : 1;
      await pool.query(
        'INSERT INTO users (id, email, password_hash, role, balance, created_at, updated_at) VALUES (?, ?, ?, "admin", 0, NOW(), NOW())',
        [nextId, email, password_hash]
      );
